export default{
    selected: null,
      zupanija: [
        { text: "Odaberi županiju", disabled: true },
        {
          value: "Bjelovarsko-bilogorska županija",
          text: "Bjelovarsko-bilogorska županija",
        },
        {
          value: "Brodsko-posavska županija",
          text: "Brodsko-posavska županija",
        },
        {
          value: "Dubrovačko-neretvanska županija",
          text: "Dubrovačko-neretvanska županija",
        },
        {
          value: "Grad Zagreb",
          text: "Grad Zagreb",
        },
        { value: "Istarska županija", text: "Istarska županija" },
        {
          value: "Karlovačka županija",
          text: "Karlovačka županija",
        },
        {
          value: "Koprivničko-križevačka županija",
          text: "Koprivničko-križevačka županija",
        },
        {
          value: "Krapinsko-zagorska županija",
          text: "Krapinsko-zagorska županija",
        },
        {
          value: "Ličko-senjska županija",
          text: "Ličko-senjska županija",
        },
        {
          value: "Međimurska županija",
          text: "Međimurska županija",
        },
        {
          value: "Osječko-baranjska županija",
          text: "Osječko-baranjska županija",
        },
        {
          value: "Požeško-slavonska županija",
          text: "Požeško-slavonska županija",
        },
        {
          value: "Primorsko-goranska županija",
          text: "Primorsko-goranska županija",
        },
        {
          value: "Sisačko-moslavačka županija",
          text: "Sisačko-moslavačka županija",
        },
        {
          value: "Splitsko-dalmatinska županija",
          text: "Splitsko-dalmatinska županija",
        },
        {
          value: "Šibensko-kninska županija",
          text: "Šibensko-kninska županija",
        },
        {
          value: "Varaždinska županija",
          text: "Varaždinska županija",
        },
        {
          value: "Virovitičko-podravska županija",
          text: "Virovitičko-podravska županija",
        },
        {
          value: "Vukovarsko-srijemska županija",
          text: "Vukovarsko-srijemska županija",
        },
        { value: "Zadarska županija", text: "Zadarska županija" },
        {
          value: "Zagrebačka županija",
          text: "Zagrebačka županija",
        },
      ],
      selectedKategorija: [],
      kategorija: [
        {
          value: "zemljaniRadovi",
          text: "Zemljani radovi",
        },
        {
          value: "betonskiIliAbRadovi",
          text: "Betonski i A-B radovi",
        },
        {
          value: "zidarskiRadovi",
          text: "Zidarski radovi",
        },
        {
          value: "tesarskiRadovi",
          text: "Tesarski radovi",
        },
        {
          value: "izolacijskiRadovi",
          text: "Izolacijski radovi",
        },
        {
          value: "krovopokrivackiRadovi",
          text: "Krovopokrivački radovi",
        },
        {
          value: "limarskiRadovi",
          text: "Limarski radovi",
        },
        {
          value: "bravarskiRadovi",
          text: "Bravarski radovi",
        },
        {
          value: "stolarskiRadovi",
          text: "Stolarski radovi",
        },
        {
          value: "keramicarskiRadovi",
          text: "Keramičarski radovi",
        },
        {
          value: "soboslikarskoLicilackiRadovi",
          text: "Soboslikarsko ličilački radovi",
        },
        {
          value: "podopolagackiRadovi",
          text: "Podopolagački radovi",
        },
        {
          value: "estrihIliPlivajuciPodovi",
          text: "Estrih ili plivajući podovi",
        },
        {
          value: "hidroinstalacije",
          text: "Hidroinstalacije",
        },
        {
          value: "elektroinstalacije",
          text: "Elektroinstalacije",
        },
        {
          value: "strojarskeInstalacije",
          text: "Strojarske instalacije",
        },
      ],
      topZupanija: [
        {
          text: "Sve županije",
          value: "",
        },
        {
          customRadio: "customRadio1",
          custom: "customRadio1",
          text: "Bjelovarsko-bilogorska županija",
          value: "Bjelovarsko-bilogorska županija",
        },
        {
          customRadio: "customRadio2",
          custom: "customRadio2",
          text: "Brodsko-posavska županija",
          value: "Brodsko-posavska županija",
        },
        {
          customRadio: "customRadio3",
          custom: "customRadio3",
          text: "Dubrovačko-neretvanska županija",
          value: "Dubrovačko-neretvanska županija",
        },
        {
          customRadio: "customRadio4",
          custom: "customRadio4",        
          text: "Grad Zagreb",
          value: "Grad Zagreb",
        },
        {
           customRadio: "customRadio5",
        custom: "customRadio5",
        text: "Istarska županija",
        value: "Istarska županija",
      },
        {
          customRadio: "customRadio6",
          custom: "customRadio6",
          text: "Karlovačka županija",
          value: "Karlovačka županija",
        },
        {
          customRadio: "customRadio7",
          custom: "customRadio7",
          text: "Koprivničko-križevačka županija",
          value: "Koprivničko-križevačka županija",
        },
        {
          customRadio: "customRadio8",
          custom: "customRadio8",
          text: "Krapinsko-zagorska županija",
          value: "Krapinsko-zagorska županija",
        },
        {
          customRadio: "customRadio9",
          custom: "customRadio9",
          text: "Ličko-senjska županija",
          value: "Ličko-senjska županija",
        },
        {
          customRadio: "customRadio10",
          custom: "customRadio10",
          text: "Međimurska županija",
          value: "Međimurska županija",
        },
        {
          customRadio: "customRadio11",
          custom: "customRadio11",
          text: "Osječko-baranjska županija",
          value: "Osječko-baranjska županija",
        },
        {
          customRadio: "customRadio12",
          custom: "customRadio12",
          text: "Požeško-slavonska županija",
          value: "Požeško-slavonska županija",
        },
        {
          customRadio: "customRadio13",
          custom: "customRadio13",
          text: "Primorsko-goranska županija",
          value: "Primorsko-goranska županija",
        },
        {
          customRadio: "customRadio14",
          custom: "customRadio14",
          text: "Sisačko-moslavačka županija",
          value: "Sisačko-moslavačka županija",
        },
        {
          customRadio: "customRadio15",
          custom: "customRadio15",
          text: "Splitsko-dalmatinska županija",
          value: "Splitsko-dalmatinska županija",
        },
        {
          customRadio: "customRadio16",
          custom: "customRadio16",
          text: "Šibensko-kninska županija",
          value: "Šibensko-kninska županija",
        },
        {
          customRadio: "customRadio17",
          custom: "customRadio17",
          text: "Varaždinska županija",
          value: "Varaždinska županija",
        },
        {
          customRadio: "customRadio18",
          custom: "customRadio18",
          text: "Virovitičko-podravska županija",
          value: "Virovitičko-podravska županija",
        },
        {
          customRadio: "customRadio19",
          custom: "customRadio19",
          text: "Vukovarsko-srijemska županija",
          value: "Vukovarsko-srijemska županija",
        },
        { customRadio: "customRadio20",
        custom: "customRadio20", 
        text: "Zadarska županija", 
        value: "Zadarska županija", 
       },
        {
          customRadio: "customRadio21",
          custom: "customRadio21",
          text: "Zagrebačka županija",
          value: "Zagrebačka županija",
        },
      ],

}